# AGENTS.md — AI Agent Instructions for Distroless FreeNGINX

## 🎯 Project Overview
Production-ready, security-hardened FreeNGINX image built from scratch with HTTP/3, QUIC, ECH, and PQC support.
...
- **Registry:** ghcr.io/ammnt/freenginx, Docker Hub: ammnt/freenginx
...
- `freenginx.conf` — main configuration
...
- No new privileges
- Seccomp profile provided (`freenginx-seccomp.json`)
...
RUN apk add freenginx  # No package managers
USER root  # Never run as root
COPY --from=builder /bin/sh /bin/sh  # No shells

# ✅ DO this
FROM scratch
COPY --from=builder /usr/local/freenginx /usr/local/freenginx
USER 10001:10001
```
...
### CI Pipeline Stages
1. **Build** — Compile FreeNGINX with hardening flags
...
### Manual (in .env)
- FreeNGINX version, OpenSSL version
...
- Issues: [GitHub Issues](https://github.com/ammnt/freenginx/issues)
